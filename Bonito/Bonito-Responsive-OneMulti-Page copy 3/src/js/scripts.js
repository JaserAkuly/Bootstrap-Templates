	...
	bonito = function() {
		return {
			...
			init: function(options) {
				...
				// AJAX CONTACT FORM 
				// ---------------------------------
				var $contactForm = $('#form-contact-us'), btnSubmit = $contactForm.find('[type="submit"]');
					...
					$contactForm.validate({
						...
						submitHandler: function(form) {
							$(form).ajaxSubmit({
								type: 'POST',
								data: $(this).serialize(),
								url: bonito.homeUrl+'ajax.php',
								beforeSend: function() {
									btnSubmit.button('loading');
								},
								success: function(data) {
									if(data.success) {
										$('#contact-form-result').removeClass('label-danger label-success').addClass('label-success').text('Sent!').fadeIn();
								...
								error: function() {
									$('#contact-form-result').removeClass('label-danger label-success').addClass('label-danger').text('Not Sent!').fadeIn();
									btnSubmit.button('reset');
									setTimeout(function(){
										$('#contact-form-result').fadeOut();
									}, 7000);
